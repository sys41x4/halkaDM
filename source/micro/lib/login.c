...
void reset_terminal(struct passwd* pwd)
{
	...
	if (pid == 0)
	{
		execl(pwd->pw_shell, pwd->pw_shell, "-c", "/usr/bin/tput reset", NULL);
		exit(EXIT_SUCCESS);
	}
...
void pam_diagnose(int error, struct term_buf* buf)
{
	switch (error)
	{
		...
		{
			buf->info_line = config.err_pam_acct_expired;
			break;
		}
		case PAM_AUTH_ERR:
		{
			buf->info_line = config.err_pam_auth;
			break;
		}
		case PAM_AUTHINFO_UNAVAIL:
		{
			buf->info_line = config.err_pam_authinfo_unavail;
			break;
		}
		case PAM_BUF_ERR:
		{
			buf->info_line = config.err_pam_buf;
			break;
		}
		case PAM_CRED_ERR:
		{
			buf->info_line = config.err_pam_cred_err;
			break;
		}
		case PAM_CRED_EXPIRED:
		{
			buf->info_line = config.err_pam_cred_expired;
			break;
		}
		case PAM_CRED_INSUFFICIENT:
		{
			buf->info_line = config.err_pam_cred_insufficient;
			break;
		}
		case PAM_CRED_UNAVAIL:
		{
			buf->info_line = config.err_pam_cred_unavail;
			break;
		}
		case PAM_MAXTRIES:
		{
			buf->info_line = config.err_pam_maxtries;
			break;
		}
		case PAM_NEW_AUTHTOK_REQD:
		{
			buf->info_line = config.err_pam_authok_reqd;
			break;
		}
		case PAM_PERM_DENIED:
		{
			buf->info_line = config.err_pam_perm_denied;
			break;
		}
		case PAM_SESSION_ERR:
		{
			buf->info_line = config.err_pam_session;
			break;
		}
		case PAM_SYSTEM_ERR:
		{
			buf->info_line = config.err_pam_sys;
			break;
		}
		case PAM_USER_UNKNOWN:
		{
			buf->info_line = config.err_pam_user_unknown;
			break;
		}
		case PAM_ABORT:
		default:
		{
			buf->info_line = config.err_pam_abort;
			break;
		}
	}
...
//        struct term_buf* buf)
{
	...
	ok = pam_start("halkaDM", NULL, &conv, &handle);
